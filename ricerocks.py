...
def click(pos):
    global started,lives,score,rockth,my_ship
...
def draw(canvas):
    global time, started,score,lives,rock_group, missile_group
    ...
    for rock in rock_group:
        for i in range(2):
             rock.vel[i] = rock.vel[i] + (rock.vel[i] * score * 0.03)
        
    #check if ship hit any of the rocks
    if lives == 0:
        started = False
        rock_group = set()
